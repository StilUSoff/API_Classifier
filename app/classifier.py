...
sys.path.append(resource_path('bin/classifier/'))
...
def visualize_grid(model, data_loader, attributes, device, checkpoint=None):
    ...
    df = pd.read_csv(resource_path('.work_labels.csv'))
...
def create_csv(directory=None,modalities=None,bodyparts=None,names=None,check=None):
    ...
    if directory is not None:
        if os.path.isfile(directory) and directory.endswith('.jpg'):
            data = [{'Name': directory, 'Modality': "CT", 'Bodypart': "Hand"}]
    ...
    with open(resource_path('.work_labels.csv'), 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(data)
    if check==1:
        os.remove(resource_path('.work_labels.csv'))
        return data
...
def main(work_folder, save=None, val=None):
    if save is None and val is None:
        ...
        attributes = AttributesDataset(resource_path('bin/classifier/val.csv'))
    else:
        checkpoint = save
    ...
    if os.path.isdir(work_folder):
        batch_size=len(os.listdir(work_folder))+1
    else:
        batch_size=1
    test_dataset = MedicalDataset(resource_path('.work_labels.csv'), attributes, val_transform)
